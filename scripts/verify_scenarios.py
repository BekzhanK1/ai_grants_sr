...
API_KEY = "X_E5h7-MiEdIPPVebcxhLZjECpGHgqvmYKo-5F_fK54"  # Must match .env
...
def check_scenario_2(tool_calls, ai_message):
    ...
    for tc in tool_calls:
        if (
            tc["tool"] == "add_interface_button"
            and int(tc["args"].get("menu_id", 0)) == 914
        ):
            found_menu = True
    ...
    missing = []
    if not found_menu:
        missing.append("menu_id=914")
    if not found_grant:
        missing.append("grant_id=350")
...
def check_scenario_4(tool_calls, ai_message):
    ...
    write_actions = [
        tc
        for tc in tool_calls
        if tc["tool"] in ["add_grant", "add_interface_button", "assign_role"]
    ]
    ...
    return (
        True,
        f"AI took {len(write_actions)} actions. Check logic manually. Actions: {[tc['args'] for tc in write_actions]}",
    )
...
def check_scenario_5(tool_calls, ai_message):
    ...
    for tc in tool_calls:
        if tc["tool"] == "add_grant":
            ...
            if gid == 382:
                found_382 = True
            if gid in VALID_DELETE_IDS:
                found_delete = True  # <--- Проверяем вхождение в список
